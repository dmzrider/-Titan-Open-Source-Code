...
- [🛡️ Permission & Logging Architecture](#-permission--logging-architecture)
...
*   🛡️ **DM Crash Proofing:** Fully fortified handlers that prevent common DM-related crashes by intercepting and rejecting server-only commands safely.
*   💾 **Advanced Env Roles:** Supports direct `.env` configuration for `OWNERS`, `ADMIN_ROLE_ID`, and `MOD_ROLE_ID` with clean fallbacks.
*   📊 **Startup Console Dashboard:** Displays loaded components, events, and commands in a beautiful, structured console box on startup.
*   📨 **Combined & Dedicated Logging:** Automated logging system for command runs, with options to combine logs or split them.
...
```text
 ...
 └── src/
     ...
     │   └── slash/            # ⚡ Native Slash commands (ping, userinfo...)
     ├── events/               # 🔔 Listener scripts (ready, messageCreate...)
     ├── handlers/             # 🛠️ Command/Event dynamic loaders
     └── utils/                # 🧰 Styling, Logging & Permission utilities
```
...
| :--- | :--- | :--- | :--- |
| `help` | None | Everyone | Displays the visual system command panel. |
| `say` | `<text>` | Moderator / Admin | Repeats text in channel (deletes original message). |
| `saye` | `<text>` | Moderator / Admin | Repeats text inside a clean embedded message. |
...
| `dm` | `<user_id> <text>`| Moderator / Admin | Sends a DM to a specific user using the bot. |
...
| Slash Command | Options | Context | Description |
| :--- | :--- | :--- | :--- |
...
## 🛡️ Permission & Logging Architecture

### 🔐 Access Levels
```text
  ┌────────────────────────────────────────────────────────┐
  │                        1. OWNER                        │  <-- Access to dmall, ann, config, etc.
  └───────────┬────────────────────────────────────────────┘
              │
              ▼
  ┌────────────────────────────────────────────────────────┐
  │                    2. ADMINISTRATOR                    │  <-- Access to announcements & setup
  └───────────┬────────────────────────────────────────────┘
              │
              ▼
  ┌────────────────────────────────────────────────────────┐
  │                      3. MODERATOR                      │  <-- Access to say, saye, dm
  └───────────┬────────────────────────────────────────────┘
              │
              ▼
  ┌────────────────────────────────────────────────────────┐
  │                       4. EVERYONE                      │  <-- Access to help, slash info, ping
  └────────────────────────────────────────────────────────┘
```
